...
def parse_headers(f):
    ...
    while 1:
        ...
        #n=line.index(':')
        #except:
        #raise dpkt.UnpackError('invalid header:%s' %line)
        if not l[0].endswith(':'):
...
class Message(dpkt.Packet,metaclass=type):
    ...
    __hdr_defaults__ = collections.OrderedDict()
    headers = None
    body = None
    ...
    def unpack(self, buf):
        try:
            ...
            self.data = f.read().encode()
        except UnicodeError as e:
     #image data .....  can't decode it ....
            f = io.StringIO(buf[:e.end-1].decode())
            buf=buf[e.end:] 
            self.headers = parse_headers(f)
            # Parse body
            # self.body = parse_body(f, self.headers)
            # Save the rest
            self.data = f.read().encode()+buf
...
class Response(Message):
    ...
    def unpack(self, buf):
        ...
        if len(l) < 2 or not l[0].startswith(self.__proto) or not l[1].isdigit():            raise dpkt.UnpackError('invalid response: %r' % line)
        self.version = l[0][len(self.__proto)+1:]
        self.status = l[1]
        self.reason = l[2]
        Message.unpack(self, f.read().encode()+buf)

    def __str__(self):
        return '%s/%s %s %s\r\n' % (self.__proto, self.version, self.status,
                                    self.reason) + Message.__str__(self)
   # def __bytes__(self):
    #    return self.__proto.self.version
...
if __name__ == '__main__':
    def test_parse_request():
        ...
        r = Request(s)
        a='sn=em&mn=dtest4&pw=this+is+atest&fr=true&login=Sign+in&od=www'
        print(r)
    ...
    def test_format_request():
        ...
        a=bytes(r)
        print(a)
        assert a == b'POST /foo/bar/baz.html HTTP/1.0\r\ncontent-length: 5\r\ncontent-type: text/plain\r\n\r\nhello'
        print(r)
