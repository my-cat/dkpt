...
def unpack_name(buf, off):
    ...
    for i in range(100): # XXX
        n = int.from_bytes(buf[off:off+1],'big')
        if n == 0:
            off += 1
            break
        elif (n & 0xc0) == 0xc0:
...
if __name__ == '__main__':
    ...
    def test_basic():
        ...
        ip = IP(s)
        dns = DNS(ip.udp.data)
        assert  dns.qd[0].name == b'www.google.com' and  dns.an[1].name == b'www.google.akadns.net'
    ...
    def test_PTR():
        ...
        dns = DNS(s)
        print(dns)
       # dns.qd[0].name == '1.1.211.141.in-addr.arpa' and
